...
do	
	...
	do
		stats=$(./waf --run "lab3 --numberOfNodes=2 --payloadSize=$payloadSize --offeredRate=11Mbps --phyRate=$phyRate --simulationTime=2 --runTcp=true" | tail -1 | cut -d ' ' -f3)
		echo $phyRate $payloadSize $stats >> lab4_2_80211_b.txt
	done
done
...
do	
	for payloadSize in 20 50 100 500 1000 1500
	do
		stats=$(./waf --run "lab3 --numberOfNodes=2 --payloadSize=$payloadSize --offeredRate=54Mbps --phyRate=$phyRate --simulationTime=2 --runTcp=true" | tail -1 | cut -d ' ' -f3)
		echo $phyRate $payloadSize $stats >> lab4_2_80211_g.txt
	done
done